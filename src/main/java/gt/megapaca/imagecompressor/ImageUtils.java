package gt.megapaca.imagecompressor;
...
public class ImageUtils {
    ...
    public static final int WATERMARK_POSITION_TOPLEFT = 1;
    public static final int WATERMARK_POSITION_TOPRIGHT = 2;
    public static final int WATERMARK_POSITION_BOTTOMLEFT = 3;
    public static final int WATERMARK_POSITION_BOOTOMRIGHT = 4;
    public static final boolean SHARPING = true;
    public static final boolean UNSHARPING = false;
    public static final boolean SCALING = true;
    public static final boolean NOSCALING = false;
...
//        }
    ...
    public static BufferedImage advancedRescaleImage(BufferedImage image, int maxWidth, int maxHeight, boolean sharping, boolean scaling) {
        // Original size
        int imageWidth = image.getWidth(null);
        int imageHeight = image.getHeight(null);
        ...
        ResampleOp resampleOp = new ResampleOp(imageWidth, imageHeight);
        if (sharping) {
     ...
     * @param toFileName The filename to use
     * @param type The image type. Use <code>ImageUtils.IMAGE_JPEG</code> to
     * @param compressFactor the % of compression, from 1 to 100, must be float
     * type. save as JPEG images, or <code>ImageUtils.IMAGE_PNG</code> to save
     * as PNG.
     * @param file is for retunt a file type
     *
     */
    public static void saveCompressedImage(BufferedImage image, String toFileName, int type, float compressFactor) {
        try {
            if (type == IMAGE_PNG) {
            ...
            iwparam.setCompressionQuality(compressFactor);

            imageWriter.write(null, new IIOImage(image, null, null), iwparam);
    ...
    public static BufferedImage addImageWatermark(BufferedImage sourceImage, BufferedImage watermarkImage, Float relativeSize, int position, int waterMarkType, float watermarkTransparency) {

//      BufferedImage sourceImage = ImageIO.read(sourceImageFile);
//      BufferedImage watermarkImage = ImageIO.read(watermarkImageFile);
        int maxWidthOri = sourceImage.getWidth(null);
        int maxHeightOri = sourceImage.getHeight(null);

        int maxWidthWM = (int) ((float) maxWidthOri * relativeSize);
        int maxHeightWM = (int) ((float) maxHeightOri * relativeSize);

        watermarkImage = advancedRescaleImage(watermarkImage, maxWidthWM, maxHeightWM, UNSHARPING, SCALING);

        maxWidthWM = watermarkImage.getWidth(null);
        maxHeightWM = watermarkImage.getHeight(null);

        // initializes necessary graphic properties
        Graphics2D g2d = (Graphics2D) sourceImage.getGraphics();
        AlphaComposite alphaChannel = AlphaComposite.getInstance(AlphaComposite.SRC_OVER, watermarkTransparency);
        g2d.setComposite(alphaChannel);

        int margin = (int) ((float) maxWidthOri * 0.01F);

        // calculates the coordinate where the image is painted
        int topLeftX = 0;
        int topLeftY = 0;

        if (position == WATERMARK_POSITION_CENTER) {
            topLeftX = (sourceImage.getWidth() - watermarkImage.getWidth()) / 2;
            topLeftY = (sourceImage.getHeight() - watermarkImage.getHeight()) / 2;
        }
        if (position == WATERMARK_POSITION_TOPLEFT) {
            topLeftX = 0 + margin;
            topLeftY = 0 + margin;
        }
        if (position == WATERMARK_POSITION_TOPRIGHT) {
            topLeftX = maxWidthOri - maxWidthWM - margin;
            topLeftY = 0 + margin;
        }
        if (position == WATERMARK_POSITION_BOTTOMLEFT) {
            topLeftX = 0 + margin;
            topLeftY = maxHeightOri - maxHeightWM - margin;
        }
        if (position == WATERMARK_POSITION_BOOTOMRIGHT) {
            topLeftX = maxWidthOri - maxWidthWM - margin;
            topLeftY = maxHeightOri - maxHeightWM - margin;
        }
